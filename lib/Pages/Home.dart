...
class _HomeState extends State<Home> {
  List<LogicalFallacy> fallacyList = [
    LogicalFallacy(
        name: "Red Herring",
        description: "Derailing an argument with an irrelevant topic/argument.",
        details:
            "A Red Herring is a fallacy in which an irrelevant topic is presented in order to divert attention from the original issue. The basic idea is to 'win' an argument by leading attention away from the argument and to another topic. This is one of the most common fallacy of all.",
        example:
    ...
    LogicalFallacy(
        ...
        example:
            "Person P presents argument X, Person Q modifies X(willingly or unwindly) to simple, easier, distorted version Y(a substitute/strawman/fake) argument of X and attacks Y. Once Y is refuted, attack on strawman is complete and hence strawman fallacy."),
    LogicalFallacy(
        name: "Ad Hominem",
        description:
            "Attacking opponent's character or personal traits in an attempt to undermine their argument.",
        details:
        ...
        example:
            "Person P presents argument X. Person Q attacks person P . Person Q concludes argument X is refuted."),
    LogicalFallacy(
        name: "Special Pleading",
        description:
            "Moving the goalposts or making up exceptions when a claim is shown to be false.",
        details:
            "Special Pleading is a fallacy in which a person applies standard principles, rules, etc. to other similar subjects but while taking themselves (or those they have a special interest in) to be exempt, without providing adequate justification for the exemption.",
        example:
            "Person P advocates Standard M for Circumstance/Situation S. When person P is in S, P concludes/claims M is not applicable anymore for P's case without consistent reasoning."),
    LogicalFallacy(
        ...
        example:
            'Person P asks question A to person Q. Question A has an assumption X which is not true. With that, person Q was made to answer WITH that assumption. Such as: "Have you stopped beating your mother?"'),
    LogicalFallacy(
        name: "Appeal to Popularity",
        description:
            "The claim where the fact that many people do or agree on something is a proof of it's validity is appeal to popularity fallacy.",
        details:
            "The basic idea is that a claim is accepted as being true simply because most people are favorably inclined towards that claim. Question: Is democracy a fallacy?",
        example:
            "Most people are in agreement with X, therefore X must be true."),
    LogicalFallacy(
        name: "Appeal to Authority",
        description:
            "Assumption that because an authority thinks something is true, it must therefore be true.",
        details:
        ...
        example:
            "Person P is an authority on subject S. P makes a claim C on S. Therefore C must be true."),
    LogicalFallacy(
        ...
        example:
            "It is inferred that either claim X is true or claim Y is true (even when X and Y could both be false). Because claim Y is false or can not be true, claim X must be true."),
    LogicalFallacy(
        ...
        example:
            "Person P: Claim X is true. Person Q: Why? Person P: Because of claim Y. Person Q: Why claim Y is true? Person P: Because claim X is true."),
    LogicalFallacy(
        ...
        example:
            "X is natural. Therefore, X is right or good. Y is unnatural. Therefore, Y is wrong or bad."),
    LogicalFallacy(
        name: "Composition",
        description:
            "Assuming that what’s true about part/parts of something is true for the whole thing.",
        details:
            "The fallacy of Composition is committed when a conclusion is drawn about a whole based on the features of its part/parts when, in fact, no justification provided for the inference.",
        example:
            "Thing A has 5 parts, P, Q, R, S, T. (1) If claim X is true for P and Q, it must be true for R, S and T, too. (2) If X is true for P, and Q, it must be true for A as a whole."),
    LogicalFallacy(
        name: "Division",
        description:
            "Assuming that what's true about a whole is true for its part/parts.",
        details:
            "The Fallacy of Division is committed when a conclusion is drawn about part/parts of something based on the features of it as a whole when, in fact, no justification provided for the inference.",
        example:
            "Thing A has 5 parts, P, Q, R, S, T. If claim X is true for A, then claim X must also be true for its parts, P, Q, R, S, and T."),
    LogicalFallacy(
        name: "Anecdotal",
        description:
        ...
        example:
            "Person P claims X is true based on statistical data. Person Q claims X isn't true in his/her own experience. Therefore X is not true."),
    ...
    LogicalFallacy(
        ...
        example:
            "Person P makes claim X. Person P asserts that P's actions or past claims are inconsistent with the truth of claim X. Therefore X is false."),
    ...
    LogicalFallacy(
        ...
        example:
            "Both are examples of this fallacy: (1) Claim X is presented by side A and the burden of proof actually rests on side B. (2) Side B claims that X is false because there is no proof for X."),
    ...
    LogicalFallacy(
        ...
        example:
            "The origin of a claim or thing is presented. The claim is true(or false) or the thing is supported (or discredited) based on the origin of the claim."),
    ...
    LogicalFallacy(
        ...
        example:
            "People believe that a claim, X, is true. Therefore X is true."),

    LogicalFallacy(
        name: "Appeal to Common Practice",
        description:
            "Claiming something is true/correct because that's how most people do it.",
        details:
    ...
    LogicalFallacy(
        ...
        description:
            "Something is true or correct only because it has been accepted correct for a long time.",
        details:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("Common Fallacies"),
        backgroundColor: Colors.grey[800],
      ),
      body: ListView.builder(
        itemCount: fallacyList.length,
        itemBuilder: (context, index) {
          LogicalFallacy currentFallacy = fallacyList[index];
          return Card(
            ...
            child: Padding(
              ...
              child: ListTile(
                onTap: () {
                  Navigator.pushNamed(context, '/singleFallacy', arguments: {
                    'title': currentFallacy.name,
                    'description': currentFallacy.description,
                    'detail': currentFallacy.details,
                    'example': currentFallacy.example
                  });
                },
                title: Text(currentFallacy.name,
...
class LogicalFallacy {
  ...
  String details;
  String example;
  LogicalFallacy({this.name, this.description, this.details, this.example});
}
